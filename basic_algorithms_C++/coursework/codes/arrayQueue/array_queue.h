...
class ArrayQueue : public Queue<T>{
    public:
        ...
        void enqueue(const T& item){
            ...
            if(head == -1) head =0;
            // if the queue is not full and tail point to the end of the array, set tail to 0
            tail = (tail+1)%storeSize;
            // else if the queue is not full and the tail does not point to the end of the array
            store[tail] = item; 
        }
        ...
        void dequeue(){
            ...
            // if head equals tail, set array to empty
            if(head > tail){
                clear();
            }
        }
